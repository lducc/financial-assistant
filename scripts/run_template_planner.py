#!/usr/bin/env python3
"""Run E009 plans over questions, joining E005b metadata by ID."""
...
def main() -> None:
    ...
    for row in read_jsonl(args.questions):
        plan = build_plan(row["question"], metadata.get(row["id"], {}))
        plans.append({"id": row["id"], **plan})
    ...
    summary = {
        ...
        "valid_plan_count": sum(row["operation"] and row["operand_roles"] for row in plans),
