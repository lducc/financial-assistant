#!/usr/bin/env python3
"""Run E007 BM25 retrieval and write auditable per-question traces."""
...
def main() -> None:
    ...
    parser.add_argument("--output-dir", type=Path, default=ROOT / "data" / "results" / "e007_bm25")
    ...
    for row in metadata_rows:
        result = retrieve(row["question"], row, reports, args.top_k)
        traces.append({"id": row["id"], "question": row["question"], "metadata": row["hard_filter"], **result})
    ...
    summary = {
        ...
        "warning": "No relevance labels were used; these are engineering diagnostics, not retrieval accuracy.",
    }
