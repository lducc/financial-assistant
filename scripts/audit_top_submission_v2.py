...
def read_rows(root: Path, record_id: int) -> list[dict[str, str]]:
    data_dir = root / "data"
    paths = sorted(data_dir.glob(f"q{record_id:04d}_df*.csv"))
...
def main() -> None:
    ...
    row_counts: list[int] = []
    query_hits = answer_hits = 0
    missing_answer_value = no_candidate = no_csv = 0
    ...
    for record in records:
        ...
        for row in rows:
            precomputed_columns.update(
                column
                for column in row
                if column in {"answer_value", "retrieval_score", "raw_number"}
            )
        ...
        if target is None:
            ...
            continue
        query_hits += 1
        if "answer_value" not in target or target.get("answer_value", "") == "":
            missing_answer_value += 1
        else:
            try:
                if abs(float(target["answer_value"]) - float(record["answer"])) <= 1e-9:
                    answer_hits += 1
        ...
        if record.get("relevant_tables") and target.get("table_id", "").strip():
            ...
            if len(examples) < 5:
                examples.append({
                    ...
                    "submitted_table": record["relevant_tables"][0],
                    "csv_table_id": target["table_id"],
                    "delta": delta,
                    "answer": record["answer"],
                    "answer_value": target.get("answer_value"),
                })
