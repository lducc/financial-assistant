#!/usr/bin/env python3
"""Run E008b and save cleaned-query row/table grounding traces."""
...
def main() -> None:
    ...
    parser.add_argument("--output-dir", type=Path, default=ROOT / "data" / "results" / "e008b_row_bm25")
    args = parser.parse_args()
    metadata_rows = [json.loads(line) for line in args.metadata.read_text(encoding="utf-8").splitlines() if line.strip()]
    if args.limit:
        metadata_rows = metadata_rows[:args.limit]
    reports = load_reports(args.dataset_root)
    traces = [{"id": row["id"], "question": row["question"], "metadata": row["hard_filter"], **retrieve_rows_clean(row["question"], row, reports, args.top_k)} for row in metadata_rows]
    args.output_dir.mkdir(parents=True, exist_ok=True)
    path = args.output_dir / "traces.jsonl"
    path.write_text("".join(json.dumps(row, ensure_ascii=False) + "\n" for row in traces), encoding="utf-8")
    summary = {
        "experiment": "E008b", "generated_at": datetime.now(UTC).isoformat(), "questions_run": len(traces),
        ...
        "trace_path": str(path), "warning": "No gold row bindings; no accuracy claim.",
    }
